...
class PhoneServiceImplTest {
    ...
    public void init() {
        ...
        setField(phoneService, "phoneRepository", phoneRepository);
    }
    ...
    void shouldCreatePhoneSuccessfully() {
        ...
        when(phoneRepository.save(any(PhoneDocument.class))).thenReturn(Mono.just(saveResult));

        var request = buildPhoneRequestBodyForTests();
        var result = phoneService.create(USER_ID, Mono.just(request));
    ...
    void shouldNotCreatePhoneSuccessfully() {
        when(phoneRepository.save(any(PhoneDocument.class))).thenReturn(Mono.error(new BadRequestExceptionReactor("Some Bad Request")));
    ...
    void shouldFindPhoneSuccessfully() {
        ...
        var result = phoneService.find(USER_ID);
    ...
    void shouldNotFindPhoneSuccessfully() {
        when(phoneRepository.findAllByUserIdAndActiveTrue(anyString())).thenReturn(Flux.error(new NotFoundExceptionReactor("Some Not Found")));
